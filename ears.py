...
class Speech():
    def __init__(
                self, robot, bhv_file, bhv_dir, log_path, 
                ):
        self.IP = robot.ip
        ...
        self.com_dict = {"speech": 
                ...
                "audio": 
                    {"robbie that is so sad", "it's time for a revolution", "test"}
                } 
        self.STOP_KEYWORD = "stop"
        # checks move is a motion proxy
        self.motion_proxy = robot.body
        self.robot = robot
        self.audio = Audio(self.IP)
        self.bhv = Behaviour(self.motion_proxy, self.robot, bhv_file, bhv_dir)
    ...
    def check_command(self, command):
        ...
        for key in self.com_dict:
            ...
            if self.STOP_KEYWORD in command:
                self.motion_proxy.run_behaviour(behaviour_name="stop")
    ...
    def greeting(self):
        self.robot.speak(random.choice(self.bhv.greeting_list))

    def emotion(self):
        self.robot.speak(random.choice(self.bhv.emotions_list))
    ...
    def run_speech(self, command):
        ...
        if command == "hello" or command == "hello robbie":
            ...
            self.robot.speak("The time is {}:{} {}".format(hour, minute, meridiem))
        elif command == "how are you" or command == "how are you robbie":
            self.emotion()

        elif command == "where are we":
            self.robot.speak("We are at Edge Hill University, situated in the north-western town of Ormskirk")
            
        elif command == "what is this department":
            self.robot.speak("this is the computer science department, which has a wide variety of courses!")
            
        elif command == "what is your favourite pathway":
            self.robot.speak("BSc Robotics and Artificial Intelligence of course!")

        elif command == "what is up dog":
            self.robot.speak("Nothimg much, whats up with you")
    ...
    def run_audio(self, command):
        ...
        if command == "robbie that is so sad":
            self.robot.speak("Wow that is so sad... Now playing despacito by Luis Fonsi")
            ...
            self.audio.play_audio(file_name="you_are_already")
    ...
    def run_mv(self, command):
        ...
        if command == "can you sit down":
            self.robot.speak("Time for a sit down!")
            self.motion_proxy.posture.sit()
            
        elif command == "stand":
            self.robot.speak("Standing up!")
            self.motion_proxy.posture.stand()
            
        elif command == "can you walk forwards" or command == "come over here robbie":
            self.robot.speak("Get ready to catch me!")
            self.motion_proxy.posture.walk(walk_duration=5)
            
        elif command == "can you walk backwards":
            self.robot.speak("Get ready to catch me!")
            self.motion_proxy.posture.walk(x=-0.5, walk_duration=5)
            
        elif command == "are you tired robbie":
            self.robot.speak("yawn... time for a lie down")
            self.motion_proxy.posture.lying_back()
...
class Behaviour():
    ...
    def __init__(self, motion_proxy=None, robot=None, f_name='', _dir = ''):
    ...
    def call_act(self, action, act_type=''):
        ...
        if act_type == "behaviour":
            self.motion_proxy.run_behaviour(behaviour_name=self.bhv_dict[action])
        elif act_type == "dance":
            self.motion_proxy.run_behaviour(behaviour_name=self.dance_dict[action])
...
if __name__ == "__main__":
    ...
    bhv_dir = "pkl_sources/"
    log_path = "extra_info/audio_log.txt"
