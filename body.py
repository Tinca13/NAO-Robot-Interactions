...
class Body():
    def __init__(self, robot, motion_proxy, am_proxy):
        ...
        self.unrefined_movement = Unrefined_Control(self.master_joint, motion_proxy)
        self.is_aware = True
    ...
    def arm_straight_up(self, l_or_r, speed=0.6):
        if l_or_r == "L":
            self.master_joint.set_pitch("LShoulder", -1.2, speed)
        elif l_or_r == "R":
            self.master_joint.set_pitch("RShoulder", -1.2, speed)
        else:
    ...
    def head_up_down(self, u_or_d, speed=0.6):
        if u_or_d == "D":
    ...
    def hand_on_hip(self, l_or_r):
        # FIXME: update to use radians rather than degrees
        if l_or_r == "L":
    ...
    def get_movements(self):
        names = self.manager_proxy.getInstalledBehaviors()
        print(names)
        return names
...
class Posture():
    ...
    def walk(self, x_dir=0.5, y_dir=0.0, theta=0.0, freq=0.5, walk_duration=3):
        ...
        self.walk_start(x_dir, y_dir, theta, freq)
        time.sleep(walk_duration)
        self.walk_stop()
    
    def walk_start(self, x_dir=0.5, y_dir=0.0, theta=0.0, freq=0.5):
